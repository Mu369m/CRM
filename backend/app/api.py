...
from .admin_schemas import (
    BonusRulePayload,
    KycRequirementPayload,
    ManagerConnectionPayload,
    ManagerConnectionResponse,
    RebateRulePayload,
)
...
async def public_tenant_config(domain: str, db: AsyncSession = Depends(get_db)):
    ...
    tenant = await db.scalar(
        select(Tenant).where(
            ...
            or_(Tenant.subdomain == domain, Tenant.custom_domain == domain),
        )
    )
    if not tenant:
    ...
    published_theme = await db.scalar(
        select(TenantThemeVersion)
        .where(
            TenantThemeVersion.tenant_id == tenant.id,
            TenantThemeVersion.status == "PUBLISHED",
        )
        .order_by(TenantThemeVersion.version.desc())
    )
    theme = published_theme.config if published_theme else {}
    settings = await db.get(TenantSettings, tenant.id)
    return TenantBrandingResponse(
        company_name=theme.get("company_name", tenant.name),
        primary_color=theme.get("primary_color", branding.primary_color),
        secondary_color=theme.get("secondary_color", branding.secondary_color),
        logo_url=theme.get("logo_url", branding.logo_url),
        favicon_url=theme.get("favicon_url", branding.favicon_url),
        meta_title=settings.meta_title if settings else tenant.name,
...
async def login(
    ...
    if user.totp_enabled and (
        not payload.two_factor_code
        or not verify_totp(
            decrypt_field(user.totp_secret_encrypted or ""), payload.two_factor_code
        )
    ):
        raise HTTPException(
...
async def me(
    ...
    user = await db.scalar(
        select(User).where(
            User.id == UUID(claims["sub"]), User.tenant_id == UUID(claims["tenant_id"])
        )
    )
    if not user:
...
async def setup_2fa(
    ...
    return TwoFactorSetupResponse(
        secret=secret,
        provisioning_uri=f"otpauth://totp/BrokerageCRM:{user.email}?secret={secret}&issuer=BrokerageCRM",
    )
...
async def verify_2fa(
    ...
    if (
        ...
        or not verify_totp(decrypt_field(user.totp_secret_encrypted), payload.code)
    ):
...
async def submit_kyc(
    ...
    claims: dict[str, str] = Depends(require_roles(Role.TRADER)),
    db: AsyncSession = Depends(get_tenant_db),
):
...
async def review_kyc(
    document_id: UUID,
    payload: KycReview,
    claims: dict[str, str] = Depends(require_roles(Role.COMPLIANCE, Role.SUPER_ADMIN)),
    db: AsyncSession = Depends(get_tenant_db),
):
    ...
    document = await db.scalar(
        ...
        .where(
            ...
            KycDocument.tenant_id == UUID(claims["tenant_id"]),
        )
        .with_for_update()
    )
    if not document:
    ...
    user = await db.scalar(
        select(User)
        .where(User.id == document.user_id, User.tenant_id == document.tenant_id)
        .with_for_update()
    )
...
async def create_money_request(
    ...
    existing = await db.scalar(
        select(MoneyRequest).where(
            MoneyRequest.idempotency_key == payload.idempotency_key,
            MoneyRequest.tenant_id == UUID(claims["tenant_id"]),
        )
    )
    if existing:
        return existing
...
async def register_trading_account(
    ...
    existing = await db.scalar(
        select(TradingAccount).where(
            ...
            TradingAccount.platform == payload.platform,
            TradingAccount.external_login == payload.external_login,
    ...
    if existing:
        return {
            ...
            "external_login": existing.external_login,
            "status": existing.provisioning_status,
        }
    account = TradingAccount(
        tenant_id=tenant_id, user_id=UUID(claims["sub"]), **payload.model_dump()
    )
    db.add(account)
    try:
        await db.commit()
        ...
        if not existing:
            raise HTTPException(
                status_code=409,
                detail="Unable to safely resolve duplicate trading account",
            )
        return {
            "id": existing.id,
            "platform": existing.platform,
...
async def operations_summary(
    ...
    traders = await db.scalar(
        select(func.count(User.id)).where(
            User.tenant_id == tenant_id, User.role == Role.TRADER
        )
    )
    pending_kyc = await db.scalar(
        select(func.count(KycDocument.id)).where(
            KycDocument.tenant_id == tenant_id, KycDocument.status == "PENDING"
        )
    )
    pending_money = await db.scalar(
        select(func.count(MoneyRequest.id)).where(
            MoneyRequest.tenant_id == tenant_id, MoneyRequest.status == "PENDING"
        )
    )
    return {
        "clients": clients or 0,
        "active_traders": traders or 0,
        "pending_kyc": pending_kyc or 0,
        "pending_treasury": pending_money or 0,
    }
...
async def update_tenant_settings(
    ...
    if not settings:
        settings = TenantSettings(
            tenant_id=UUID(claims["tenant_id"]), **payload.model_dump()
        )
        db.add(settings)
    else:
...
async def upsert_rebate_rule(
    payload: RebateRulePayload,
    claims: dict[str, str] = Depends(require_roles(Role.SUPER_ADMIN)),
    db: AsyncSession = Depends(get_tenant_db),
):
    ...
    rule = await db.scalar(
        select(RebateRule).where(
            ...
            RebateRule.instrument_group == payload.instrument_group,
            RebateRule.level == payload.level,
        )
    )
    if rule:
        for key, value in payload.model_dump().items():
...
async def list_rebate_rules(
    claims: dict[str, str] = Depends(require_roles(Role.SUPER_ADMIN, Role.SALES)),
    db: AsyncSession = Depends(get_tenant_db),
):
    """List tenant rebate rules for review and partner operations."""
    rules = await db.scalars(
        select(RebateRule)
        .where(RebateRule.tenant_id == UUID(claims["tenant_id"]))
        .order_by(RebateRule.level, RebateRule.instrument_group)
    )
    return list(rules)


@router.put("/admin/kyc-requirements", response_model=KycRequirementPayload)
async def upsert_kyc_requirement(
    payload: KycRequirementPayload,
    claims: dict[str, str] = Depends(require_roles(Role.SUPER_ADMIN, Role.COMPLIANCE)),
    db: AsyncSession = Depends(get_tenant_db),
):
    ...
    requirement = await db.scalar(
        select(KycRequirement).where(
            KycRequirement.tenant_id == tenant_id,
            KycRequirement.document_type == payload.document_type,
        )
    )
    if requirement:
        for key, value in payload.model_dump().items():
...
async def create_manager_connection(
    payload: ManagerConnectionPayload,
    claims: dict[str, str] = Depends(require_roles(Role.SUPER_ADMIN)),
    db: AsyncSession = Depends(get_tenant_db),
):
    ...
    credentials = json.dumps(
        {"username": payload.username or payload.login, "password": payload.password}
    )
    connection = ManagerConnection(
        ...
        login=payload.login,
        encrypted_password=encrypt_field(credentials),
...
@router.get(
    "/admin/manager-connections", response_model=list[ManagerConnectionResponse]
)
async def list_manager_connections(
    claims: dict[str, str] = Depends(require_roles(Role.SUPER_ADMIN, Role.FINANCE)),
    db: AsyncSession = Depends(get_tenant_db),
):
    """List manager metadata without exposing encrypted credentials."""
    connections = await db.scalars(
        select(ManagerConnection)
        .where(ManagerConnection.tenant_id == UUID(claims["tenant_id"]))
        .order_by(ManagerConnection.created_at.desc())
    )
    return list(connections)


@router.post(
    "/admin/manager-connections/{connection_id}/accounts",
...
async def create_external_account(
    ...
    connection = await db.scalar(
        select(ManagerConnection).where(
            ManagerConnection.id == connection_id,
            ManagerConnection.tenant_id == UUID(claims["tenant_id"]),
            ManagerConnection.enabled.is_(True),
        )
    )
    if not connection:
        raise HTTPException(
            status_code=404, detail="Enabled manager connection not found"
        )
...
async def wallet(
    ...
    wallet = await db.scalar(
        select(Wallet).where(
            Wallet.owner_id == UUID(claims["sub"]),
            Wallet.tenant_id == UUID(claims["tenant_id"]),
        )
    )
    if not wallet:
...
async def adjust_wallet(
    ...
    signed_amount = (
        adjustment.amount
        if adjustment.entry_type.name in {"DEPOSIT", "COMMISSION", "ADJUSTMENT"}
    ...
    db.add(
        AuditLog(
            ...
            metadata_json=adjustment.model_dump_json(),
        )
    )
    await db.commit()
