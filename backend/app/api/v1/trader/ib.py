...
def _ib_withdrawal_reference(request_id: UUID) -> str:
    return f"ib-withdrawal:{request_id}"


async def partner_for(db: AsyncSession, claims: dict[str, str]) -> IbPartner:
    partner = await db.scalar(
        select(IbPartner).where(
            IbPartner.user_id == UUID(claims["sub"]),
            IbPartner.tenant_id == UUID(claims["tenant_id"]),
        )
    )
    if not partner:
...
async def overview(
    ...
    direct_ids = select(User.id).where(
        User.parent_ib_id == user_id, User.tenant_id == tenant_id
    )
    referred = (
        await db.scalar(
            select(func.count()).select_from(User).where(User.id.in_(direct_ids))
        )
        or 0
    )
    volume = await db.scalar(
        select(func.coalesce(func.sum(Position.volume), 0)).where(
            Position.trader_id.in_(direct_ids), Position.tenant_id == tenant_id
        )
    ) or Decimal("0")
    ...
    return {
        ...
        "referral_link": f"https://{host}/trader/register?ref={partner.referral_code}",
        "referred_traders": referred,
        "direct_active_volume": volume,
        "total_earned_commissions": earned,
...
async def network(claims: Claims, db: AsyncSession = Depends(get_tenant_db)):
    ...
    for user in users:
        if user.parent_ib_id:
            children.setdefault(user.parent_ib_id, []).append(user)
        ...
        return [
            {
                ...
                "role": child.role,
                "level": level,
                "children": walk(child.id, level + 1),
            }
...
async def commissions(
    ...
    total = await db.scalar(
        select(func.count(LedgerEntry.id)).where(
            LedgerEntry.wallet_id == wallet.id, LedgerEntry.entry_type == "COMMISSION"
        )
    )
...
async def withdraw(
    ...
    if not wallet or wallet.balance < payload.amount:
        raise HTTPException(status_code=409, detail="Insufficient IB wallet balance")
    ...
    request = MoneyRequest(
        ...
        currency="USD",
        status="PENDING",
        idempotency_key=f"ib-{user_id}-{uuid4()}",
    )
    db.add(request)
    db.add(
        LedgerEntry(
            ...
            amount=-payload.amount,
            reference=_ib_withdrawal_reference(request_id),
            note=payload.destination,
        )
    )
    await db.commit()
    return {
