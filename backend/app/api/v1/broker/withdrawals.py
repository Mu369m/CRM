...
@router.post("/methods", response_model=WithdrawalMethodResponse, status_code=status.HTTP_201_CREATED)
...
async def create_withdrawal(
    ...
    result = await db.execute(
        select(WithdrawalMethod).where(
            (WithdrawalMethod.id == payload.method_id) & (WithdrawalMethod.tenant_id == tenant_id)
        )
    )
    method = result.scalar()
    if not method:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Withdrawal method not found")
    ...
    if client.net_deposits and client.net_deposits < payload.amount:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def approve_withdrawal(
    ...
    result = await db.execute(
        select(ClientFinancials).where(ClientFinancials.client_id == withdrawal.client_id)
    )
    ...
    client = result.scalar()
    if client:
        client.total_withdrawals = (client.total_withdrawals or Decimal("0")) + withdrawal.net_amount
        client.net_deposits = (client.net_deposits or Decimal("0")) - withdrawal.net_amount
