...
BrokerClaims = Annotated[dict[str, str], Depends(require_roles(Role.SUPER_ADMIN, Role.BROKER_ADMIN))]
SelectionType = Literal["ALL", "FILTERED", "SPECIFIC"]
BulkAction = Literal[
    "TRADING_ENABLE", "TRADING_DISABLE", "BUY_ENABLE", "BUY_DISABLE",
    "SELL_ENABLE", "SELL_DISABLE", "EA_ENABLE", "EA_DISABLE",
    "SET_MAX_LOT", "SET_MAX_OPEN_POSITIONS", "KILL_SWITCH",
]
...
class BulkTradingRequest(BaseModel):
    ...
    action: BulkAction
    max_lot_size: Decimal | None = Field(default=None, gt=0, max_digits=20, decimal_places=8)
...
def _filters(tenant_id: UUID, search: str | None, platform: AccountPlatform | None):
    ...
    if search:
        term = f"%{search.strip()}%"
        conditions.append(or_(TradingAccount.external_login.ilike(term), TradingAccount.server.ilike(term)))
    if platform:
...
def _selection_query(payload: BulkTradingRequest, tenant_id: UUID):
    ...
    if payload.selection_type == "SPECIFIC":
        if not payload.account_ids:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Specific selection requires account_ids")
        conditions.append(TradingAccount.id.in_(payload.account_ids))
    elif payload.account_ids:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="account_ids are only valid for SPECIFIC selection")
...
async def list_trading_accounts(
    ...
    items = list(await db.scalars(select(TradingAccount).where(*conditions).order_by(TradingAccount.created_at.desc()).offset(offset).limit(limit)))
    ...
    return TradingAccountPage(items=items, total=total or 0, selection_type=selection_type, offset=offset, limit=limit)


@router.post("/accounts/bulk", response_model=dict)
async def bulk_trading_action(payload: BulkTradingRequest, claims: BrokerClaims, db: AsyncSession = Depends(get_tenant_db)) -> dict:
    tenant_id, actor_id = UUID(claims["tenant_id"]), UUID(claims["sub"])
    accounts = list(await db.scalars(_selection_query(payload, tenant_id).with_for_update()))
    affected_count = len(accounts)
    if affected_count == 0:
        return {"status": "NO_MATCHES", "affected_count": 0, "successful": 0, "failed": 0, "failed_records": []}
    dangerous = payload.action.endswith("DISABLE") or payload.action == "KILL_SWITCH"
    if dangerous and not payload.confirm:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail={"code": "CONFIRMATION_REQUIRED", "affected_count": affected_count, "action": payload.action})
    if payload.action == "SET_MAX_LOT" and payload.max_lot_size is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="max_lot_size is required")
    if payload.action == "SET_MAX_OPEN_POSITIONS" and payload.max_open_positions is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="max_open_positions is required")
    ...
    for account in accounts:
        ...
        changes.append({"entity_id": str(account.id), "previous": previous, "new": new_values})

    audit_rows = [{"tenant_id": tenant_id, "actor_id": actor_id, "action": f"TRADING:{payload.action}", "metadata_json": json.dumps({"entity_type": "TRADING_ACCOUNT", "entity_id": change["entity_id"], "selection_type": payload.selection_type, "affected_count": affected_count, "previous_value": change["previous"], "new_value": change["new"], "reason": payload.reason})} for change in changes]
    await db.execute(insert(AuditLog), audit_rows)
    await db.commit()
    return {"status": "COMPLETED", "affected_count": affected_count, "successful": affected_count, "failed": 0, "failed_records": [], "selection_type": payload.selection_type}
