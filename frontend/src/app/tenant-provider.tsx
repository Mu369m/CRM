...
export function TenantProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const applyBranding = (nextBranding: TenantBranding) => {
    ...
    const handleThemeUpdate = (event: MessageEvent<{ type?: string; branding?: TenantBranding }>) => {
      if (event.data?.type === "theme_updated" && event.data.branding) applyBranding(event.data.branding);
    };
    channel?.addEventListener("message", handleThemeUpdate);
    return () => {
      channel?.removeEventListener("message", handleThemeUpdate);
      channel?.close();
    };
  }, []);
