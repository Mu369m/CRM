...
export default function MainLayout({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#070A11] text-slate-100 lg:flex">
    ...
    <div className="min-w-0 flex-1 pb-20 lg:pb-0"><header className="flex h-16 items-center justify-between border-b border-slate-800/80 bg-[#0D121F]/90 px-4 backdrop-blur lg:px-8"><button className="grid size-9 place-items-center rounded border border-slate-700 text-slate-300 lg:hidden" onClick={() => setDrawerOpen(true)} aria-label="Open navigation"><Menu size={18} /></button><div className="lg:hidden"><Brand compact /></div><div className="hidden text-[10px] uppercase tracking-[.18em] text-slate-500 lg:block">Broker administration workspace</div><div className="flex items-center gap-3"><ThemeToggle /><span className="hidden items-center gap-2 text-xs text-emerald-300 sm:flex"><span className="size-2 rounded-full bg-emerald-400" /> Secure session</span><span className="grid size-8 place-items-center rounded-full bg-cyan-400/15 text-[10px] font-bold text-cyan-200">RK</span></div></header><div className="p-4 sm:p-6 lg:p-8">{children}</div></div>
